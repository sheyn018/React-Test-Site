import { useLayoutEffect, useCallback, useEffect, useState, useMemo, useRef } from "react";
...
const ChakraCarousel = ({ children, gap, itemWidthInitial }) => {
  const [trackIsActive, setTrackIsActive] = useState(false);
  const [multiplier, setMultiplier] = useState(0.35);
  const [sliderWidth, setSliderWidth] = useState(0);
  const [activeItem, setActiveItem] = useState(0);
  const [constraint, setConstraint] = useState(0);
  const [itemWidth, setItemWidth] = useState(itemWidthInitial);
...
const Slider = ({
  ...
  const chevronBoxSize = width < 600 ? 4 : 6;
  ...
  return (
    <>
      ...
      {numItems > 1 && activeItem <= positions.length - constraint && (
        <Flex w={`${itemWidth}px`} mt={`${gap / 2}px`} mx="auto">
          <Button
            ...
            minW={0}
          >
            <ChevronLeftIcon boxSize={chevronBoxSize} />
          </Button>
          <Progress
          ...
          <Button
            ...
            minW={0}
          >
            <ChevronRightIcon boxSize={chevronBoxSize} />
          </Button>
        </Flex>
      )}
    </>
...
const Track = ({
  ...
  const handleClick = useCallback(
    (event) =>
      node.current && (node.current as any).contains(event.target)
        ? setTrackIsActive(true)
